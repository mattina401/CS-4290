...
void SMPCache::doRead(MemRequest *mreq)
{
	...
	bool compMissFlag = false;
	bool capMissFlag = false;
	PAddr tag = calcTag(addr);
	std::set<PAddr>::iterator temp;



	//comMiss Handling
	//CompList contains tag

	//bool check = std::find(compList.begin(), compList.end(), tag) != compList.end();
	if(compList.find(tag) == compList.end())
		compMissFlag = true;
	else
	{

		if(capList.size()-1 >= cache->getNumLines())
			capMissFlag = false;
		if(capList.find(tag) == capList.begin()) {
			capMissFlag = true;
			temp = capList.find(tag);
		}
		else
			capMissFlag = false;
	...
	capList.insert(capList.begin(), tag);
...
void SMPCache::doWrite(MemRequest *mreq)
{
	...
	PAddr tag = calcTag(addr);
	std::set<PAddr>::iterator temp;
	//std::set<PAddr>::iterator tagTemp;
